import { ParsedNote, ParsedScore } from "../parser/interfaces.js";
import {
  CachedMeasureInfo,
  MeasureBoundary,
  Performance,
...
import PlayerPartitionProcessor from "./PlayerPartitionProcessor.js";
import TieProcessor from "./TieProcessor.js";
import EmptyNoteSetsProcessor from "./EmptyNoteSetsProcessor.js";
import { PitchToByte } from "./Utils.js";
import SimplificationProcessor from "./SimplificationProcessor.js";
...
export default class ScoreBuilder {
  ...
  constructCachedNoteSetsByMeasure(
    ...
    function processOutputCachedMeasureInfo({
      ...
      const matchingNoteSetTimes = Object.entries(matchingNoteSetTimesMap)
        ...
        .map(([noteSetTime]) => parseFloat(noteSetTime))
        .sort((a, b) => a - b);
  ...
  build(input: ParsedScore): Performance {
    ...
    const output2 = new TieProcessor(output1).process();
    const output3 = new SimplificationProcessor(output2).process();
    const output4 = new EmptyNoteSetsProcessor(output3).process();

    const finalOutput = output4;
    Object.freeze(finalOutput);
